...
class ToolApi:
    ...
    def save_not_executed_cases_list(self):
        ...
        executed_cases_ids_set: set[int] = self._get_executed_cases_ids_list()
        not_executed_cases_ids_list: list[int] = self._get_not_executed_cases_ids(cases_ids, executed_cases_ids_set)

        not_executed_cases_list: list[TestCase] = self._get_test_cases_list_by_id_list(
            not_executed_cases_ids_list)

        self._write_info_about_not_executed_cases_list(not_executed_cases_list)

    def save_most_failing_test_cases(self):
        print("Getting the most failing test cases in test runs...")

        failed_tests_with_extended_info_list: list[TestInRunWithCaseInfo] = self._get_failed_tests_with_extended_info(
            self.__api_requests.failed_tests)

        test_cases_usage_info: list[tuple[int, int]] = self._get_test_case_usage_info(
            failed_tests_with_extended_info_list)

        self._write_info_about_most_failing_test_cases(test_cases_usage_info, failed_tests_with_extended_info_list)
    ...
    def save_the_buggiest_tests(self):
        ...
        self._save_info_about_the_buggiest_tests(failed_tests_with_bugs_list)

    def _save_info_about_the_buggiest_tests(self, failed_tests_with_bugs_list: list[TestInRun]):
        failed_tests_with_at_least_one_bug_list: list[TestInRun] = self._get_failed_tests_with_at_least_one_bug_list(
            failed_tests_with_bugs_list)

        formatted_all_defects_list: list[str] = self._get_all_defects_list(failed_tests_with_at_least_one_bug_list)
        most_common_defects: list[tuple[str, int]] = self._get_most_common_defects(formatted_all_defects_list)

        test_case_with_bugs_list: list[dict[str, int or list[str]]] = self._get_test_case_with_bugs_list(
            failed_tests_with_at_least_one_bug_list)

        bug_with_test_cases: list[dict[str, str or list]] = self._get_bug_with_test_cases(most_common_defects,
                                                                                          test_case_with_bugs_list)

        self._write_info_about_the_buggiest_tests(bug_with_test_cases)

    def _write_info_about_not_executed_cases_list(self, not_executed_cases_list: list[TestCase]):
    ...
    def _write_info_about_most_failing_test_cases(self, test_cases_usage_info: list[tuple[int, int]],
        ...
        self._write_result_logs(result_file_name)

    def _write_info_about_the_buggiest_tests(self, bug_with_test_cases: list[dict[str, str or list]]):
    ...
    def _get_test_case_by_test(self, test: TestInRun) -> TestCase or None:
    ...
    def _get_executed_cases_ids_list(self) -> set[int]:
        executed_cases_set = self._get_test_cases_list_by_id_list(
    ...
    @staticmethod
    def _get_test_case_usage_info(failed_tests_list: list[TestInRunWithCaseInfo]) -> list[tuple[int, int]]:
    ...
    def _get_failed_tests_with_extended_info(self, failed_tests_list: list[TestInRun]) -> list[TestInRunWithCaseInfo]:
        failed_tests_with_extended_info: list[TestInRunWithCaseInfo] = []

        for failed_test in failed_tests_list:
            test_case: TestCase = self._get_test_case_by_test(failed_test)
    ...
    @staticmethod
    def _get_not_executed_cases_ids(cases_ids: set[int], executed_cases_ids_set: set[int]) -> list[int]:
    ...
    def _get_test_cases_list_by_id_list(self, id_list: list[int] or set[int]) -> list[TestCase]:
        return [case for case in self.__api_requests.cases if case.id in id_list]

    def _get_tests_with_defects_list(self) -> list[TestInRun]:
    ...
    def _get_failed_tests_with_bugs_list(self, tests_with_defects_list: list[TestInRun]) -> list[TestInRun]:
        # merge failed tests with bugs list with information about test cases
        # example result: {'id': 123, 'case_id': 1234, 'status_id': 5, 'test_id': 12345, 'defects': ['JIRA-123']}
        if len(tests_with_defects_list) <= len(self.__api_requests.failed_tests):
            return self._get_intersected_failed_tests_with_bugs_list(
                self.__api_requests.failed_tests, tests_with_defects_list)
        else:
            return self._get_intersected_failed_tests_with_bugs_list(tests_with_defects_list,
                                                                     self.__api_requests.failed_tests)
    ...
    @staticmethod
    def _get_failed_tests_with_at_least_one_bug_list(failed_tests_with_bugs_list: list[TestInRun]) -> \
            list[TestInRun]:
    ...
    @staticmethod
    def _get_test_case_with_bugs_list(failed_tests_with_at_least_one_bug_list: list[TestInRun]) -> \
            list[dict[str, int or list[str]]]:
        return [{"case_id": failed_test.case_id, "defects": failed_test.defects} for failed_test in
                failed_tests_with_at_least_one_bug_list]

    def _get_bug_with_test_cases(self, most_common_defects: list[tuple[str, int]],
